...
class GAMMA(object):
    def __init__(self,dimension, map_cstr=None, num_pe=64, pe_limit=1024, fitness="latency", constraints=dict(), par_RS=False, l1_size=512, l2_size=108000, NocBW=81920000, offchipBW=81920000, slevel_min=2,slevel_max=2, fixedCluster=0, log_level=2,constraint_class=None,external_mem_cstr=None, use_factor=False,uni_base=True,external_area_model=True):
        super(GAMMA,self).__init__()
        self.external_area_model = external_area_model  # Ensure this is True
    ...
    def create_genome_fixedSL(self, bias=None):
        ...
        # Generate a minimal valid solution
        ind = [
            ['C', 1],  # Spatial mapping dimension (smallest possible)
            ['K', 1], ['C', 1], ['Y', 1], ['X', 1], ['R', 1], ['S', 1],  # L2 tiles
            ['C', 1], ['K', 1], ['Y', 1], ['X', 1], ['R', 1], ['S', 1]   # L1 tiles
        ]

        # Ensure cluster hierarchy matches slevel_min
        for _ in range(self.slevel_min - 1):
            ind = self.born_cluster_ind(ind)

        # Enforce PE limit
        if self.pe_limit > 0:
            ind[0][1] = min(ind[0][1], self.pe_limit)  # Cap PE count
    ...
    def mutate_tile(self, pop, is_finetune=False, num_mu_loc=1, alpha=0.5, range_alpha=0.5, cluster_only=False):
        for idx in range(len(pop)):
            ...
            for _ in range(num_mu_loc):
                if random.random() < alpha:
                    if self.map_cstr:
                        if self.cstr_list and len(self.cstr_list) > 1:  # Modified line
                            num_free_tile = self.cstr_list[1].get("num_free_tile", 0)
                        else:
                            num_free_tile = 0  # Default value
                        if num_free_tile==0:
    ...
    def mutate_pe(self, pop, alpha=0.7, mutate_range_ratio=0.5):
        """Mutate the number of PEs to explore different configurations."""
        for idx in range(len(pop)):
            if random.random() < alpha:
            # Get current spatial dimension and tile size
                sp_dim = pop[idx][0][0]
                last_cluster = self.scan_indv(pop[idx][:7])
                max_possible_pe = self.dimension_dict[sp_dim] // last_cluster[sp_dim]

                # Ensure PE stays within hardware limits
                new_pe = random.randint(1, min(max_possible_pe, self.pe_limit))
                pop[idx][0][1] = new_pe
            
                # Debug print
                # print(f"[mutate_pe] Changed PE count: {new_pe} (Max possible: {max_possible_pe})")
    ...
    def evaluate(self, pool, population, cur_gen=-1):
        ...
        for i in range(len(population)):
            result = reward_activ_list[i]
            if result is None:
                reward = [float("-Inf") for _ in range(len(self.best_reward))]
                activity_count = None
                count_non_valid += 1
            else:
                reward, activity_count = result
    ...
    def calculate_energy(self, macs, l1_accesses, l2_accesses, dram_accesses, 
                   mac_energy=0.2, l1_energy=5, l2_energy=10, dram_energy=100):
        """
        Calculate total energy in nanojoules (nJ).
        Default energy values in pJ/op:
        - mac_energy: 0.2pJ/MAC (INT8)
        - l1_energy: 5pJ/access
        - l2_energy: 10pJ/access  
        - dram_energy: 100pJ/access
        """
        total = (macs * mac_energy + 
                l1_accesses * l1_energy + 
                l2_accesses * l2_energy + 
    ...
    def compute_area_external(self, num_pe, l1_size, l2_size):
        MAC_AREA_INT8 = 282  # µm²/PE
        BUF_AREA_perbit = 0.086  # µm²/bit
        bits_per_element = 8  # 8 bits per element (1 byte)

        # Debug inputs
        # print(f"[DEBUG] compute_area_external inputs: num_pe={num_pe}, l1_size={l1_size}, l2_size={l2_size}")

        # Calculate buffer areas
        l1_bits = l1_size * num_pe * bits_per_element
        l2_bits = l2_size * bits_per_element
        buf_area = (l1_bits + l2_bits) * BUF_AREA_perbit  # µm²

        # Calculate PE area
        pe_area = num_pe * MAC_AREA_INT8  # µm²

        # Total area in mm²
        total_area = (pe_area + buf_area)   # Convert µm² → mm²
    ...
    def generate_neighbors(self, current_solution, num_neighbors=10):
        """Generate neighboring solutions by perturbing the current solution."""
        neighbors = []
        for _ in range(num_neighbors):
            neighbor = copy.deepcopy(current_solution)

            # Initialize constraints for Tabu Search
            if self.map_cstr:  # Add this block
                self.cstr_list, self.num_free_order, self.num_free_par = \
                    self.map_cstr.get_cstr_list(neighbor, self.fixedCluster)
                
            # Log original values
            original_pe = neighbor[0][1]
            original_spatial_dim = neighbor[0][0]

            # print("\n=== Neighbor Generation ===")
            # print("Original PE:", neighbor[0][1])
            
            # Apply mutation operators
            self.mutate_tile([neighbor], num_mu_loc=1, alpha=0.7)
            self.swap_order([neighbor], alpha=0.7)
            # self.mutate_pe([neighbor], alpha=0.5)

            # print("New PE:", neighbor[0][1])

            # Log changes
            # print(f"\n--- Neighbor {_+1} ---")
            # print(f"PE: {original_pe} → {neighbor[0][1]}")
            # print(f"Spatial Dim: {original_spatial_dim} → {neighbor[0][0]}")
            
            # Ensure validity
            self.correctify_tile_dependency([neighbor])
            self.comform_to_cstr([neighbor])
            reward, maestro_output = self.oberserve_maestro(neighbor)

            # Ensure memory accesses are non-negative
            # for key in maestro_output:
            #     maestro_output[key] = np.abs(maestro_output[key])  # Convert negative values to positive

            
             # Print neighbor details and reward
            # print(f"\n--- Neighbor {_} ---")
            # print("Mapping:", neighbor)
            # # print("PE Count:", neighbor[0][1])
            # print("Reward:", reward[0] if reward else "INVALID (None)")
            # print("Energy:", reward[1] if reward else "INVALID (None)")
            
            if reward is not None:
                neighbors.append(neighbor)
            else:
                _-=1  # Retry generating this neighbor if invalid
                # print("Invalid neighbor, retrying...")
        return neighbors

    def tabu_search(self, pool, initial_solution, max_iterations=100, tabu_tenure=10):
        ...
        for iteration in range(max_iterations):
            ...
            for neighbor in neighbors:
                self.comform_to_cstr([neighbor])  # Enforce constraints
                reward, _ = self.oberserve_maestro(neighbor)
                if reward is not None and all(r != float("-Inf") for r in reward):
                    valid_neighbors.append((neighbor, reward))
            
            # Filter tabu solutions
            non_tabu_neighbors = [
                (n, r) for n, r in valid_neighbors 
                if self.solution_hash(n) not in tabu_list
            ]
            
            if not non_tabu_neighbors:
                # Aspiration criteria: override tabu if better than global best
                best_candidate = max(valid_neighbors, key=lambda x: x[1][0], default=None)
                if best_candidate and best_candidate[1][0] > best_reward[0]:
                    current_solution, reward = best_candidate
                else:
                    break
            else:
                # Select best non-tabu neighbor
                current_solution, reward = max(non_tabu_neighbors, key=lambda x: x[1][0])
            ...
            # Logging
            if self.log_level > 1:
                runtime, _, _, area, l1_size, l2_size, _, _, num_pe = self.get_indiv_info(current_solution)
                print(f"Iter {iteration+1}: Reward: {reward[0]:.3e}, Runtime: {runtime}, Area: {area/1e6:.3f}mm², PE: {num_pe}")

            if best_reward is None:
                print("Error: No valid solution found within iterations.")
                exit(1)

            runtime, _, _, area, l1_size, l2_size, _, _, num_pe = self.get_indiv_info(best_solution)
            print(f"Best Reward: {best_reward[0]:.3e}, Runtime: {runtime:.0f} cycles, Area: {area/1e6:.3f} mm²")
        return best_solution, best_reward

    def solution_hash(self, solution):
        """Create a hashable representation of a solution."""
        return str([[item for sublist in solution for item in sublist]])