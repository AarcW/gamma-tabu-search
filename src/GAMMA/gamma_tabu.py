...
class GAMMA(object):
    def __init__(self,dimension, map_cstr=None, num_pe=64, pe_limit=1024, fitness="latency", constraints=dict(), par_RS=False, l1_size=512, l2_size=108000, NocBW=81920000, offchipBW=81920000, slevel_min=2,slevel_max=2, fixedCluster=0, log_level=2,constraint_class=None,external_mem_cstr=None, use_factor=False,uni_base=True):
        ...
        # In the GAMMA class initialization
        self.fitness_objective = ["latency", "area"]  # Ensure "area" is included
    ...
    def evaluate_solution(self, solution):
        ...
        if not self.validTo_external_mem_cstr(solution):
            print("Invalid solution!")  # ✅ DEBUG LINE
            return float("-inf")
        
        pool = None  # No multiprocessing
        reward_activ_list = self.thread_fun(solution)
        reward, _ = reward_activ_list
        print(f"Evaluating: {solution}, Reward: {reward}")  # ✅ DEBUG LINE
        ...
        for _ in range(5):  # Generate 5 neighbor solutions
            ...
            if move_type == "tile":
                ...
                if isinstance(new_solution[idx][1], int):
                    new_solution[idx][1] = max(1, int(new_solution[idx][1] * random.uniform(0.8, 1.2)))  # Wider range for more diversity
            elif move_type == "pe":
                pe_idx = next((i for i, x in enumerate(new_solution) if x[0] == "PE"), None)
                if pe_idx is not None:
                    new_solution[pe_idx][1] = max(1, new_solution[pe_idx][1] + random.choice([-2, -1, 1, 2]))  # More variation in PE count
                ...
                if l1_idx is not None and l2_idx is not None:
                    shift_amount = random.randint(-200, 200)  # Larger shift for more diversity
                    ...
                    print(f"Updated L1 size: {new_solution[l1_idx][1]}, L2 size: {new_solution[l2_idx][1]}")  # Debugging
            
            # Validate the new solution before adding it to the neighbors list
            if self.validTo_external_mem_cstr(new_solution):
                neighbors.append(new_solution)
                print(f"Generated neighbor: {new_solution}")  # ✅ DEBUG LINE
            else:
                print(f"Invalid neighbor: {new_solution}")  # ✅ DEBUG LINE
    ...
    def run_tabu_search(self, dimension, num_iterations=100, tabu_size=10):
        ...
        if not self.validTo_external_mem_cstr(current_solution):
            print("Initial solution is invalid! Regenerating...")
            current_solution = self.create_genome_fixedSL()  # Regenerate until a valid solution is found
    ...
    def reset_hw_parm(self, l1_size=None, l2_size=None, num_pe=None, NocBW=None, map_cstr=None, pe_limit=None,area_pebuf_only=None, external_area_model=None, offchipBW=None):
        if l1_size:
            self.l1_size = l1_size if l1_size > 0 else 2**30
            print(f"Reset L1 size: {self.l1_size}")  # Debugging
        if l2_size:
            self.l2_size = l2_size if l2_size > 0 else 2**30
            print(f"Reset L2 size: {self.l2_size}")  # Debugging
        if num_pe:
            self.num_pe=num_pe
        if NocBW:
            self.NocBW=NocBW if NocBW > 0 else 2**30
    ...
    def oberserve_maestro(self, indv, num_pe=None, l1_size=None, l2_size=None, NocBW=None, offchipBW=None):
        print(f"Observing maestro: l1_size={l1_size}, l2_size={l2_size}")  # Debugging
    ...
    def compute_area_external(self, num_pe, l1_size, l2_size):
        print(f"Computing area: num_pe={num_pe}, l1_size={l1_size}, l2_size={l2_size}")  # Debugging
        MAC_AREA_INT8 = 282
        BUF_AREA_perbit = 0.086
        buf_size = l1_size * num_pe + l2_size
        area = num_pe * MAC_AREA_INT8 + buf_size * BUF_AREA_perbit * 8
        print(f"Computed area: {area}")  # Debugging
        return area
