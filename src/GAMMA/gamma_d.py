...
class GAMMA(object):
    ...
    def initialize_tabu_search(self, initial_solution=None):
        if initial_solution is None:
            initial_solution = self.create_genome()  # Use existing function to generate a random solution
        tabu_list = []
        best_solution = initial_solution
        best_fitness = self.evaluate(pool, [best_solution])[0]
        return initial_solution, tabu_list, best_solution, best_fitness

    def generate_neighbors(self, current_solution):
        neighbors = []
        for i in range(len(current_solution)):
            # Change tile size for dimension i
            dim, tile_size = current_solution[i]
            new_tile_size = tile_size + 1  # Example: increment tile size
            if new_tile_size <= self.dimension_dict[dim]:  # Ensure tile size is valid
                neighbor = copy.deepcopy(current_solution)
                neighbor[i][1] = new_tile_size
                neighbors.append(neighbor)
        return neighbors

    def stopping_criteria_met(self, best_fitness, iteration, no_improvement_threshold=10, previous_best_fitness=None):
    
    # Determines whether the stopping criteria for Tabu Search has been met.
    
    # Args:
    #     best_fitness (float): The best fitness value found so far.
    #     iteration (int): The current iteration number.
    #     no_improvement_threshold (int): The number of iterations to wait without improvement before stopping.
    #     previous_best_fitness (float): The best fitness value from the previous iteration.
    
    # Returns:
    #     bool: True if the stopping criteria is met, False otherwise.

        if previous_best_fitness is None:
            # Initialize previous_best_fitness if not provided
            previous_best_fitness = best_fitness
        
        # Check if there has been no improvement for the specified number of iterations
        if iteration > no_improvement_threshold and best_fitness == previous_best_fitness:
            return True
        return False
...
def tabu_search(self, max_iterations=100, tabu_size=10):
    # Create a pool for evaluation (even if single-threaded)
    with Pool(1) as pool:
        current_solution, tabu_list, best_solution, best_fitness = self.initialize_tabu_search(pool)
        previous_best_fitness = best_fitness
        ...
        for iteration in range(max_iterations):
            ...
            for neighbor in neighbors:
                if neighbor not in tabu_list:
                    # Evaluate the neighbor using the pool
                    fitness = self.evaluate(pool, [neighbor])[0]
