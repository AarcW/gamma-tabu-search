...
import ast  # To safely parse mapping strings
...
def parse_metrics(output):
    """Extract metrics and mapping from Gamma output using regex"""
    metrics = {
    ...
    # Extract mapping
    map_match = re.search(r"Mapping:\s*(\[\[.*?\]\])", output, re.DOTALL)
    if map_match:
        try:
            ...
            results["Mapping"] = "Parse Error"
    else:
        results["Mapping"] = "Not Found"
...
# Parse results and compute EDP
original_metrics = parse_metrics(original_stdout)
original_metrics["Execution Time (s)"] = original_time
if "Runtime (cycles)" in original_metrics and "Energy (nJ)" in original_metrics:
    original_metrics["EDP (cycles·nJ)"] = original_metrics["Runtime (cycles)"] * original_metrics["Energy (nJ)"]
else:
    original_metrics["EDP (cycles·nJ)"] = "N/A"

tabu_metrics = parse_metrics(tabu_stdout)
tabu_metrics["Execution Time (s)"] = tabu_time
if "Runtime (cycles)" in tabu_metrics and "Energy (nJ)" in tabu_metrics:
    tabu_metrics["EDP (cycles·nJ)"] = tabu_metrics["Runtime (cycles)"] * tabu_metrics["Energy (nJ)"]
else:
    tabu_metrics["EDP (cycles·nJ)"] = "N/A"

# Print results to terminal
print("\n--- Original Gamma Output ---")
for key, value in original_metrics.items():
    print(f"{key}: {value}")

print("\n--- Tabu Search Gamma Output ---")
for key, value in tabu_metrics.items():
    print(f"{key}: {value}")
